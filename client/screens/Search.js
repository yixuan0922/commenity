...
export default class Search extends React.Component {
    ...
    handleSearchChange = (search) => {
        ...
        const results = this.state.responseData.filter((item) => search && item.title.toLowerCase().includes(search));
        ...
        return (
            <Block style={styles.notfound}>
                <Text style={{ fontFamily: "open-sans-regular" }} size={18} color={argonTheme.COLORS.TEXT}>
                    We didn’t find "<Text bold>{this.state.search}</Text>" posts.
                </Text>

                {/* <Text
                    size={18}
                    style={{ marginTop: theme.SIZES.BASE, fontFamily: "open-sans-regular" }}
                ...
                renderItem={({ item }) => (
                    ...
                    >
                        <Block flex row middle space="between">
                            <Text style={{ fontFamily: "open-sans-regular" }} size={14} color={argonTheme.COLORS.TEXT}>
                                {item.title}
                            </Text>
                            <Icon name="chevron-right" family="evilicon" style={{ paddingRight: 5 }} />
                        </Block>
                    </TouchableOpacity>
                )}
            />
    ...
    renderDeals = () => {
        return (
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.dealsContainer}>
                <Block flex>
                    <Block flex row>
    ...
    renderResult = (result, index) => {
        ...
        return (
            <Animated.View style={{ width: width - theme.SIZES.BASE * 2, opacity }} key={`result-${result.title}`}>
                ...
                <Card item={result} idx={index} horizontal></Card>
            </Animated.View>
        );
    };
    ...
    render() {
        return (
            <Block flex center style={styles.searchContainer}>
                ...
                <ScrollView showsVerticalScrollIndicator={false}>{this.renderResults()}</ScrollView>
            </Block>
        );
    }
}
